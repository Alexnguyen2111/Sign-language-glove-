 ...
 * Date: 01/12/2025 (Updated: 2025-05-20)
 *
...
int counter = 0;
unsigned long lastFlexDebugPrintTime = 0; // For printing flex debug info periodically
...
void speakWithVoiceRSS(String textToSpeak) {
        ...
        if (httpCode > 0) {
            ...
            if (httpCode == HTTP_CODE_OK) {
                ...
                if (stream == nullptr) {
                    ...
                    // http.end() will be called in finally block
                } else {
    ...
    isSpeaking = false; // Clear busy flag
}
...
void loadDataset() {
    ...
    if (!file) {
        Serial.printf("Lỗi: Không thể mở tệp %s để đọc. Không có dữ liệu huấn luyện nào được tải.\n", csvFilename);
    ...
    for (int i = row; i < Y_Traind_Max_Size; ++i) Y_Traind[i].label = "";
    file.close();
}
...
void setup() {
    ...
    if (!LittleFS.begin(true)) {
        Serial.println("Lỗi khởi tạo LittleFS! Không thể lưu/tải dữ liệu."); return;
    }
...
void loop() {
    ...
    if ((millis() - lastTime) > gyroDelay) {
        ...
        DataCSV_ML currentGestureInstance; 
